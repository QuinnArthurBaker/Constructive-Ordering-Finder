...
# This file was *autogenerated* from the file beginning_subsets.sage
...
def check_subsets(subset, n):
	running_sum = _sage_const_0 
	already_seen_nums = []
	for i in subset:
		running_sum+=i
		running_sum%=n
		if running_sum in already_seen_nums or running_sum == _sage_const_0 :
...
def main(size):
	subsets = map(lambda x: x[:int(len(x)/_sage_const_2 )],itertools.permutations(range(_sage_const_1 ,size))) #get the half-sized subsets for verification
	good_subsets = []
